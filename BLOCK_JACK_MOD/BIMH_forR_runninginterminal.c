...
// create shared file using R CMD SHLIB -lgsl -lgslcblas BIMH_forR.c distributions.c minimum.c  // need Makevar file

void serial_block_IMH(double* px_0, size_t* pp, size_t* pb, double (*x_mat)[((*pp)*(*pb))+1], double (*w_mat)[((*pp)*(*pb))+1], double* pest, double* x_chain, double* w_chain)
{
  printf("hello\n");
...
void serial_block_IMH(double* px_0, size_t* pp, size_t* pb, double (*x_mat)[((*pp)*(*pb))+1],
		      double (*w_mat)[((*pp)*(*pb))+1], double* pest, double* x_chain, double* w_chain);
...
int main()
{
  ...
  double x_mat[p][(p*b) + 1];
  double w_mat[p][(p*b) + 1];

  double* pest;
  est = 0.0;
  pest = &est;
  double x_chain[(p*b) + 1];
  double w_chain[(p*b) + 1];
  printf("hello2\n");
  serial_block_IMH(px_0, pp, pb, x_mat,  w_mat, pest, x_chain, w_chain);
  return 0;
}
